...
class EmployeeDepartmentControllerTest {
    ...
    void getAllEmployeesReturnsFour() throws Exception {
        mvc.perform(get("/employees").with(httpBasic("user", "pwd"))).andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(4));
    }

    @Test
    void getAllDepartmentsReturnsTwo() throws Exception {
        mvc.perform(get("/departments").with(httpBasic("user", "pwd"))).andExpect(status().isOk())
    ...
    void updateEmployeeChangesTheName() throws Exception {
        ...
        mvc.perform(put("/employees").with(httpBasic("user", "pwd"))
    ...
    void deleteEmployeeRemovesItFromTheList() throws Exception {
        mvc.perform(delete("/employees/4").with(httpBasic("user", "pwd"))).andExpect(status().isOk());
        mvc.perform(get("/employees").with(httpBasic("user", "pwd")))
                .andExpect(jsonPath("$.length()").value(3));
    }

    @Test
    void deleteUnknownEmployeeReturns404() throws Exception {
        mvc.perform(delete("/employees/99").with(httpBasic("user", "pwd"))).andExpect(status().isNotFound());
    }

    @Test
    void postCountryEchoesBackTheValidPayload() throws Exception {
        String payload = "{\"code\":\"FR\",\"name\":\"France\"}";
        mvc.perform(post("/countries").with(httpBasic("user", "pwd"))
                        .contentType(MediaType.APPLICATION_JSON).content(payload))
                .andExpect(status().isOk())
